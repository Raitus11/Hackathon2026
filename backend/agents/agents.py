...
def provisioner_agent(state: dict) -> dict:
    ...
    for idx, qm in enumerate(sorted_qms):
        ...
        lines.append(f"*")
        lines.append(f"* Channel naming convention: <source_qm>.<target_qm>")
        lines.append(f"*   (IBM 20-char channel-name limit; dot-separator preferred")
        lines.append(f"*    over .TO. which consumes 3 chars unnecessarily)")
        lines.append(f"*")
        lines.append(f"* Recovery if anything fails mid-run:")
        lines.append(f"*   - Connect via:     runmqsc {qm}")
        lines.append(f"*   - Inspect status:  DISPLAY CHSTATUS(*)")
        lines.append(f"*   - Inspect queues:  DISPLAY QSTATUS(*) IPPROCS CURDEPTH")
        lines.append(f"*   - Restart channel: START CHANNEL(<name>)")
        lines.append(f"*   - Stop channel:    STOP CHANNEL(<name>) MODE(QUIESCE)")
        lines.append(f"*   - Reset XMITQ:     RESET CHANNEL(<name>)")
        lines.append(f"*")
        lines.append(f"* For idempotent re-runs use this script as-is (REPLACE on every DEFINE).")
        lines.append(f"* For surgical updates use the matching {qm}_diff.mqsc script.")
        lines.append(f"* =============================================================")
        lines.append("")
    ...
    # ── Generate diff MQSC scripts (B3) ───────────────────────────────────
    # For each QM, produce a {qm}_diff.mqsc that shows ONLY the deltas
    # between as-is and target. Veterans run diff scripts, not full
    # re-applies — REPLACE on existing channels can disturb running
    # connections. The diff script is what an SRE actually executes.
    #
    # If as_is_graph is missing (rare — should always be in state by this
    # point), we emit a placeholder explaining why and pointing to the
    # full target script.
    per_qm_diff_scripts = _generate_diff_mqsc_scripts(
        as_is_graph=state.get("as_is_graph"),
        target_graph=G,
        per_qm_target_scripts=per_qm_scripts,
        qm_port_map=qm_port_map,
        port_base=port_base,
    )
    ...
    # B3: Also add per-QM diff scripts (deltas only, what SREs actually run)
    for qm, diff_script in per_qm_diff_scripts.items():
        target_csvs[f"mqsc_{qm}_diff"] = diff_script
...
# ─────────────────────────────────────────────────────────────────────────────
# B3 — Per-QM diff MQSC generation
# ─────────────────────────────────────────────────────────────────────────────
#
# What this does: for each QM, compute the channel delta between as-is and
# target, then emit an MQSC script that performs ONLY those deltas. This is
# what an SRE actually runs in production. Veterans don't `runmqsc < target.mqsc`
# on a live system — REPLACE on an active SDR channel disturbs running
# connections. They run `runmqsc < diff.mqsc` which only touches what changed.
#
# What "delta" means here:
#   ADDED   — channel exists in target_graph but not in as_is_graph.
#             Emit DEFINE CHANNEL ... NOREPLACE so we don't accidentally
#             clobber a manually-created channel of the same name.
#   REMOVED — channel exists in as_is but not in target.
#             Emit STOP CHANNEL + DELETE CHANNEL with QUIESCE mode so
#             in-flight messages drain.
#   UNCHANGED — same (from_qm, to_qm) in both. NOT emitted. The whole
#             point of the diff is to leave these alone.
#
# Note: queues, listeners, remote queue definitions are NOT diffed in
# Phase 1. They're idempotent under REPLACE and rarely the source of
# production incidents. Channels are. Phase 2 can add full-object diffing
# if SREs ask for it.
# ─────────────────────────────────────────────────────────────────────────────


def _generate_diff_mqsc_scripts(
    as_is_graph,
    target_graph,
    per_qm_target_scripts: dict,
    qm_port_map: dict,
    port_base: int = 1414,
) -> dict:
    """Generate {qm}_diff.mqsc for each QM in target_graph.

    Returns dict: {qm_name: diff_script_string}.

    If as_is_graph is None (rare — should always be present), every QM
    gets a placeholder script that explains why no diff was generated
    and points to the full target script.
    """
    target_qms = sorted(
        n for n, d in target_graph.nodes(data=True) if d.get("type") == "qm"
    )
    diff_scripts: dict[str, str] = {}

    # ── Placeholder path: as_is_graph missing ─────────────────────────────
    if as_is_graph is None:
        for qm in target_qms:
            lines = [
                "* =============================================================",
                f"* Diff MQSC for {qm} — PLACEHOLDER",
                "*",
                "* No as-is topology was available when this script was generated.",
                "* A real diff requires comparing source-state channels to target",
                "* state to compute adds/removes; without the as-is graph the",
                "* provisioner cannot determine which channels are new vs existing.",
                "*",
                f"* Run the full script instead: runmqsc {qm} < {qm}_target.mqsc",
                "* (Use this approach only on greenfield QMs — REPLACE on a live",
                "* system can disturb running connections.)",
                "* =============================================================",
                "",
            ]
            diff_scripts[qm] = "\n".join(lines)
    ...
    # ── Real diff path ────────────────────────────────────────────────────
    # Index as-is and target channels by (from_qm, to_qm).
    def _channels_by_endpoint(g):
        idx: dict[tuple, dict] = {}
    ...
    as_is_chans = _channels_by_endpoint(as_is_graph)
    target_chans = _channels_by_endpoint(target_graph)

    as_is_keys = set(as_is_chans.keys())
    target_keys = set(target_chans.keys())

    added_keys = target_keys - as_is_keys
    removed_keys = as_is_keys - target_keys
    unchanged_keys = as_is_keys & target_keys

    for qm in target_qms:
        # Channels to ADD on this QM (this QM is the sender side, i.e. from_qm == qm)
        adds_on_qm = sorted(k for k in added_keys if k[0] == qm)
        # Channels to REMOVE on this QM (this QM was the sender side in as-is)
        removes_on_qm = sorted(k for k in removed_keys if k[0] == qm)
        # Receivers also need diffs — channels into this QM that are added or removed
        adds_inbound_on_qm = sorted(k for k in added_keys if k[1] == qm)
        removes_inbound_on_qm = sorted(k for k in removed_keys if k[1] == qm)

        # If nothing changes for this QM, emit a no-op script (still useful —
        # tells the SRE that this QM doesn't need touching).
        nothing_to_do = not (
            adds_on_qm or removes_on_qm
            or adds_inbound_on_qm or removes_inbound_on_qm
        )

        lines = []
        lines.append("* =============================================================")
        lines.append(f"* Diff MQSC for {qm}")
        lines.append("* Generated by IntelliAI Provisioner Agent")
        lines.append(f"* Run via: runmqsc {qm} < {qm}_diff.mqsc")
        lines.append("*")
        lines.append("* This script applies ONLY the channel deltas between the")
        lines.append("* current state and the target topology. Use this on a live")
        lines.append("* QM in preference to the full target script — REPLACE on")
        lines.append("* an active SDR channel disturbs running connections, but")
        lines.append("* this script only touches channels that actually changed.")
        lines.append("*")
        lines.append(f"* Outbound additions:    {len(adds_on_qm)}")
        lines.append(f"* Outbound removals:     {len(removes_on_qm)}")
        lines.append(f"* Inbound additions:     {len(adds_inbound_on_qm)}")
        lines.append(f"* Inbound removals:      {len(removes_inbound_on_qm)}")
        lines.append("*")
        lines.append("* Channel naming convention: <source_qm>.<target_qm>")
        lines.append("*   (IBM 20-char channel-name limit; dot-separator preferred)")
        lines.append("*")
        lines.append("* Recovery if anything fails mid-run:")
        lines.append(f"*   - Connect via:     runmqsc {qm}")
        lines.append(f"*   - Inspect status:  DISPLAY CHSTATUS(*)")
        lines.append(f"*   - Restart channel: START CHANNEL(<name>)")
        lines.append(f"*   - Stop channel:    STOP CHANNEL(<name>) MODE(QUIESCE)")
        ...
        if nothing_to_do:
            lines.append(f"* No channel changes needed for {qm}.")
            lines.append("* This QM's channel set is identical between as-is and target.")
            lines.append("")
            lines.append("* --- End of diff script ---")
            diff_scripts[qm] = "\n".join(lines)
            continue

        # ── REMOVALS first ────────────────────────────────────────────────
        # We stop+delete BEFORE adding new ones so we don't accidentally
        # leave the QM in a state with extra channels during the rollout.
        # QUIESCE mode lets in-flight messages drain.
        if removes_on_qm:
            lines.append("* --- Remove obsolete sender channels ---")
            for from_qm, to_qm in removes_on_qm:
                ch = as_is_chans[(from_qm, to_qm)]
                lines.append(f"STOP CHANNEL('{ch['channel_name']}') MODE(QUIESCE)")
                lines.append(f"DELETE CHANNEL('{ch['channel_name']}')")
            lines.append("")

        if removes_inbound_on_qm:
            lines.append("* --- Remove obsolete receiver channels ---")
            for from_qm, to_qm in removes_inbound_on_qm:
                ch = as_is_chans[(from_qm, to_qm)]
                lines.append(f"STOP CHANNEL('{ch['channel_name']}') MODE(QUIESCE)")
                lines.append(f"DELETE CHANNEL('{ch['channel_name']}')")
            lines.append("")
        ...
        # ── ADDITIONS ─────────────────────────────────────────────────────
        # NOREPLACE so if a channel of the same name was created manually
        # (e.g. by an SRE during incident response), we surface the conflict
        # instead of silently overwriting it. SRE has to investigate.
        if adds_on_qm:
            lines.append("* --- Add new sender channels ---")
            for from_qm, to_qm in adds_on_qm:
                ch = target_chans[(from_qm, to_qm)]
                to_hostname = f"{to_qm.lower().replace('_', '-')}.target.corp.com"
                to_port = qm_port_map.get(to_qm, port_base)
                lines.append(
                    f"DEFINE CHANNEL('{ch['channel_name']}') CHLTYPE(SDR) "
                    f"CONNAME('{to_hostname}({to_port})') "
                    f"XMITQ('{ch['xmit_queue']}') NOREPLACE"
                )
            lines.append("")
            lines.append("* --- Start new sender channels ---")
            for from_qm, to_qm in adds_on_qm:
                ch = target_chans[(from_qm, to_qm)]
                lines.append(f"START CHANNEL('{ch['channel_name']}')")
            lines.append("")

        if adds_inbound_on_qm:
            lines.append("* --- Add new receiver channels ---")
            for from_qm, to_qm in adds_inbound_on_qm:
                ch = target_chans[(from_qm, to_qm)]
                lines.append(
                    f"DEFINE CHANNEL('{ch['channel_name']}') CHLTYPE(RCVR) NOREPLACE"
                )
            lines.append("")
